...
def _split_train_test(series: pd.Series, test_ratio: float = 0.2) -> Tuple[pd.Series, pd.Series]:
    n = len(series)
    test_n = max(1, int(round(n * test_ratio)))
    return series.iloc[: n - test_n], series.iloc[n - test_n :]


def _rolling_forecast(train: pd.Series, test: pd.Series, order: Tuple[int, int, int]) -> np.ndarray:
    history = train.copy()
    preds = []
...
def main() -> None:
    ...
    parser.add_argument("--model", default="ml/models/arima_model.pkl")
    parser.add_argument("--out", default="docs/screenshots/arima_actual_vs_predicted.png")
    ...
    preds = _rolling_forecast(train, test, order=order)
    ...
    plt.plot(x, preds[:n_plot], label="Predicted AQI (ARIMA)", linewidth=2)
    plt.title(f"ARIMA Actual vs Predicted AQI | order={order} | MAE={mae:.2f} RMSE={rmse:.2f}")
    ...
    print(f"order={order} mae={mae:.3f} rmse={rmse:.3f} test_points={len(y_true)}")
